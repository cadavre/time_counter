...
################ Constructor ################
    ...
    def __init__(self, hass, object_id, name, icon, restore, duration, initial, min, max, debounce):
        ...
        self._boundary_calibration = BOUNDARY_CALIBRATION / 1000 # additional time to add while reaching boundary to calibrate range on min/max
...
################ Component methods ################
    ...
    async def async_set(self, state):
        ...
        additional_time = 0
        if state == self._min or state == self._max:
            additional_time = self._boundary_calibration
        ...
        count_time = count_time + additional_time
